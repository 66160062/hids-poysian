...
async function populateShareLinks(rounds: RoundView[]) {
  for (const round of rounds) {
    if (round.statusKey === 'APPROVED') {
      try {
        const { data } = await api.get(`/auth/generate-link?project_id=${jobId.value}&role=customer`);
        const routeData = router.resolve({ path: '/customer', query: { token: data.token } });
        round.customerShareLink = window.location.origin + routeData.href;
      } catch (e) {
        console.error('Failed to generate customer token', e);
